...
const getProvinceHaveMovieByDay = async () => {
  try {
    ...
    const {data} = await axios.get(`http://localhost:8080/movies/${props.movieId}/cities?date=${selectedDate.value}`)
...
const getTheaterByCityAndDay = async () => {
  try {
    loadingStateStoreRef.setLoadingState(true)
    const {data} = await axios.get(`http://localhost:8080/movie-theaters/available/theaters?city=${encodeURIComponent(selectedCity.value)}&movieId=${props.movieId}&date=${selectedDate.value}`)
    theaters.value = data
  } catch (e) {
    console.log(e)
  } finally {
...
const getShowTimeByTheater = async () => {
  try {
    loadingStateStoreRef.setLoadingState(true)
    const {data} = await axios.get(`http://localhost:8080/showtimes/available?movieId=${props.movieId}&theaterId=${selectedTheaterId.value}&date=${selectedDate.value}`)
