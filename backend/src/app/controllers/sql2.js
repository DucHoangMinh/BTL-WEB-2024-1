...
(async () => {
  try {
    ...
    const sql = `
      ...
      BEGIN
          -- Lấy danh sách tất cả các showtime_id và room_id từ bảng Showtime trong khoảng showtime_id từ 101 đến 112
          FOR showtime_record IN
              SELECT sh.id AS showtime_id, sh.room_id
              FROM "Showtime" sh
              WHERE sh.id BETWEEN 101 AND 112 -- Điều kiện lọc showtime_id trong khoảng từ 101 đến 112
          LOOP
