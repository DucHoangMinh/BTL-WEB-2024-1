...
export default {
  ...
  methods: {
    ...
    async getReportByDateRange(startDate, endDate){
      try {
        const { data } = await axios.get(`http://160.191.50.189:8080/revenue/date-range?startDate=${this.startDate}&endDate=${this.endDate}`);
    ...
    async getMonthReport(year){
      try {
        const { data } = await axios.get(`http://160.191.50.189:8080/revenue/year?year=${year}`);
